...
try {
    ...
    choco/refres-env/refresh-env.cmd
    # test git
...
if ($start_devs_playground -ine 'n' -And $start_devs_playground -ine 'no') { 
    ...
    $host.UI.RawUI.BackgroundColor = "Black"
    $devs_playground = "$git_dir/scripts/wsl-import.cmd"
    &$devs_playground = cmd /c start powershell -Command "$git_dir/scripts/wsl-import.cmd" -WindowStyle "Maximized"
}
