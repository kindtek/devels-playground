...
function run_devels_playground {
    ...
    try {
        ...
        if (!(Test-Path -Path "$git_path/.dvlp-installed" -PathType Leaf)) {
            ...
            dir
            Write-Host "`r`npowershell.exe -Command `"dvlp/scripts/wsl-docker-import.cmd $img_subset`"`r`n"
            powershell.exe -Command "dvlp/scripts/wsl-docker-import.cmd" $img_subset
