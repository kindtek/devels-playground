...
#               ||||               Executing ...               ||||                 #
#                -------------------------------------------------                   #
docker buildx build --target dvlp_kernel-output --output type=local,dest=/mnt/c/users/"${username:-$2}"/k-cache --build-arg KERNEL_TYPE=basic KERNEL_FEATURE=zfs --build-arg REFRESH_REPO=yes --build-arg CONFIG_FILE= . 2>&1
# no-cache
# docker buildx build --no-cache --target dvlp_kernel-output --output type=local,dest=/mnt/c/users/"${username:-$2}" --build-arg KERNEL_TYPE=basic KERNEL_FEATURE=zfs --build-arg REFRESH_REPO=yes --build-arg CONFIG_FILE= . 2>&1
