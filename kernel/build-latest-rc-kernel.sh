...
sudo cp /hel/dvl-works/dvl-playg/kernel/6_3-rc/amd/.config_0 .config && \
sudo chown $LOGNAME:$(id -gn) .config && \
yes "" | sudo make prepare scripts && \
sudo make -j16 && sudo make modules_install && \
sudo mkdir -p ../../../${_AGL}/kernels/6_3-rc/amd && sudo cp -f arch/x86/boot/bzImage ../../../${_AGL}/kernels/6_3-rc/amd/linux-6_3rc_$kernel_tag && \
sudo mkdir -p ../../../dvl/kernels/6_3-rc/amd && sudo cp -f arch/x86/boot/bzImage ../../../dvl/dvl-works/dvl-playg/kernel/6_3-rc/amd/linux-6_3rc_0 && \
