...
sudo cp /hel/dvl-works/dvpg/kernels/ubuntu/x86/6_3-rc/amd/.config_0 .config && \
sudo chown $LOGNAME:$(id -gn) .config && \
yes "" | sudo make prepare scripts && \
sudo make -j16 && sudo make modules_install && \
sudo mkdir -p ../../../${_AGL}/kernels/ubuntu/x86/amd/6_3-rc && sudo cp -f arch/x86/boot/bzImage ../../../${_AGL}/kernels/ubuntu/x86/amd/6_3-rc/linux-6_3rc_$kernel_tag && \
