...
sudo cp -arf dwork/mnt/backup-dvl.sh ${backup_mnt_location}/dvl/backup-dvl.sh && \
...
sudo cp -arf dwork/mnt/backup-dvl.sh /home/dvl/backup-dvl.sh && \
...
# echo "!!!!!!!!!!!!!!!!DO NOT SAVE YOUR FILES IN THIS DIRECTORY!!!!!!!!!!!!!!!!\n\nThe dvl can/will delete your files if you save them in this directory. Keep files out of the dvls grasp and in the *${username}* sub-directory.\n\n!!!!!!!!!!!!!!!!DO NOT SAVE YOUR FILES IN THIS DIRECTORY!!!!!!!!!!!!!!!!" | sudo tee ${backup_mnt_location}/README_ASAP      && \
...
cp -arf --backup=$VERSION_CONTROL --update /home/dvl $HALO_DESTINATION;
...
cp --backup=$VERSION_CONTROL --remove-destination -arf $_NIX_MNT_LOCATION/$backup_mnt_location/dvl-$WSL_DISTRO_NAME/dvl /home" > $HALO_RESTORE_SCRIPT;
